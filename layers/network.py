...
class SpectralTrendRefineBlock(nn.Module):
    def __init__(self, seq_len, pred_len, dropout=0.1):
        ...
        self.spectral_mlp = nn.Sequential(
            nn.Linear(2 * self.F, 8),
            nn.GELU(),
            nn.Linear(8, 2 * self.F)
        )

        self.gate = nn.Parameter(torch.ones(2 * self.F))
    ...
    def forward(self, x):
        ...
        freq = torch.cat([real, imag], dim=-1)  # [B, C, 2F]

        freq = self.spectral_mlp(freq)
        ...
        x_time_refined = torch.fft.irfft(x_freq, n=T, dim=-1)
