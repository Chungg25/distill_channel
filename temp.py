...
class GroupChannelBlock(nn.Module):
    def __init__(self, seq_len, pred_len,
        ...
        self.head = nn.Sequential(
            ...
            # nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(d_model*expand, pred_len)
        )
...
class SpectralTimeBlock(nn.Module):
    def __init__(
        ...
        self.proj = nn.Sequential(
            ...
            nn.Linear(seq_len, seq_len*expand),
            # nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(seq_len*expand, pred_len)
        )
...
class Network(nn.Module):
    def __init__(self, seq_len, pred_len, patch_len, stride, padding_patch,
        ...
        # ... (Khúc đầu giữ nguyên y hệt của bạn) ...
        self.pred_len = pred_len    
        ...
        # ------------------------------------------------------------------
        # KHÔNG ĐƯỢC XÓA: Dán nhãn thời gian để Transformer không bị Mù
        # self.position_encoding = nn.Parameter(torch.randn(1, self.patch_num, d_model) * 0.02)
        # ------------------------------------------------------------------
        ...
        # ==================================================================
        # BỘ DECODER: GATED CONTEXT-AWARE LINEAR (ĐỈNH CAO CỦA LINEAR)
        # ==================================================================
        # 1. Linear Cơ bản
        self.patch_forecast = nn.Linear(self.patch_num, self.future_patch_num)
        
        # 2. Bộ tạo Bối cảnh (Bắt mạch quá khứ)
        self.context_generator = nn.Linear(d_model, self.future_patch_num)
        
        # 3. Cổng sàng lọc thông tin
        self.forecast_gate = nn.Linear(self.patch_num, self.future_patch_num)
        
        # 4. Bàn ủi Conv1d chà nhám ranh giới
        ...
        # KHÔNG ĐƯỢC XÓA: Trọng số trộn 3 nhánh
        # self.fusion_branches = nn.Parameter(torch.tensor([0.1, 0.8, 0.1]))
    ...
    def forward(self, x1, x2, current_epoch=0, max_epochs=1):
        ...
        # -------------------------------------------------------------
        # Đi qua Transformer (Cộng Position để giữ đúng trật tự thời gian)
        s_patch = s_patch
        s_patch = self.transformer_encoder(s_patch)
        x = self.repr_norm(s_patch) # [BC, P, d_model]

        # ==================================================================
        # NGHỆ THUẬT GIẢI MÃ LINEAR
        # ==================================================================
        
        # BƯỚC 1: Đọc vị bối cảnh hiện tại
        last_state = x[:, -1, :] # Lấy trạng thái mới nhất: [BC, d_model]
        context = self.context_generator(last_state).unsqueeze(1) # [BC, 1, Future_P]

        # BƯỚC 2: Phóng Linear cơ sở
        x_t = x.transpose(1, 2) # [BC, d_model, P]
        base_forecast = self.patch_forecast(x_t) # [BC, d_model, Future_P]

        # BƯỚC 3: Trộn Bối cảnh và Ép qua Cổng (Gate)
        forecast = base_forecast + context # Cộng Bối cảnh vào Dự báo
        gate = torch.sigmoid(self.forecast_gate(x_t)) # Mở cổng
        future_features = forecast * gate # [BC, d_model, Future_P]

        # BƯỚC 4: Chà nhám ranh giới bằng Conv1d
        x_conv = self.smoothing_conv(future_features)
        future_features = future_features + x_conv  # Residual làm mượt
        
        # BƯỚC 5: Giải mã
        future_features = future_features.transpose(1, 2) # [BC, Future_P, d_model]
        x_decoded = self.patch_decoder(future_features)   # [BC, Future_P, patch_len]
        
        x = x_decoded.reshape(B * C, self.future_patch_num * self.patch_len)
        temporal = x.view(B, C, -1)[:, :, :self.pred_len]
        # ==================================================================

        spectral = self.spectral(x2).view(B, C, self.pred_len)
        x = temporal + channel + spectral
        x = x.permute(0,2,1)
        # KHÔNG ĐƯỢC XÓA: Trộn 3 nhánh có kiểm soát

        aux_losses = {}
        if self.training:
            aux_losses['decorrelation'] = self.decor_loss(channel, temporal, spectral)
            aux_losses.update(self.seasonal_channel.get_aux_losses())
            adaptive_w = self.aux_weighter.compute_weights(
                aux_losses, current_epoch=current_epoch, max_epochs=max_epochs
            )
